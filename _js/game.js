	...
	var Game = function(){
		...
		*/
		var startGame = function(){
			...
			if (level > 4){
				numBubbles = MAX_BUBBLES - 20 -((level-4) * 2);
			}else{
			...
			//colorsBoolean = [true,true,true,true];  //true if color still on board
			iColorsLeft = [0,1,2,3]; //all 4 colors of bubbles presumed to exist at start
			// [green, blue, turquoise, red]
		...
		var getNextBubble = function(){
			var top;
			var left;
			//comment out this line from book code:
			//var bubble = BubbleShoot.Bubble.create();

			//now use function that only allows colors that currentyly exist on board
			var type = getNextBubbleType();
			bubble = BubbleShoot.Bubble.create(undefined,undefined,type);
		...
		var getNextBubbleType = function(){
		
		/*check for non-existant bubble color */
			MAX_BUBBLES_FOR_TYPE_CHECK = 30;
			var i;
			var typeExists = false;
			var activeBubbles = board.getBubbles();
			var bLength = activeBubbles.length;
			//iColorsLeft is an array colors of the Bubble.types left on the board
			//the Bubble types (integers) have a one-to-one relationship to the colors (integers)
			var type = iColorsLeft[Math.floor(Math.random() * iColorsLeft.length)];
			var bubble;
			if (bLength < MAX_BUBBLES_FOR_TYPE_CHECK){
				do {
					i = 0;
					do {
						if (activeBubbles[i].getType() === type){
							typeExists = true;
						};				
					  i += 1;
					}	while (i < bLength & typeExists === false);

					if (typeExists === false){
						iColorsLeft.splice(iColorsLeft.indexOf(type),1);
						type = iColorsLeft[Math.floor(Math.random() * iColorsLeft.length)];						
					};
				} while (iColorsLeft.length > 1 & typeExists === false );
			};
			return type;			
		};
		...
		*/	
			...
			if(collision){
				...
				if(colorGroup.list.length >= 3){
					...
					//here, DELAY_PER_BUBBLE_POP = hard coded 60 in book
					orphanDelay = collision.distToCollision + DELAY_PER_BUBBLE_POP * colorGroup.list.length;
					if(orphanedBubbles.length > 0){
						...
						//POP_SETUP_DELAY = 200;
						//DELAY_PER_BUBBLE_POP = 60;
						orphanDelay =  duration + POP_SETUP_DELAY + colorGroup.list.length * DELAY_PER_BUBBLE_POP;
		...
		var popBubbles = function(bubbles, delay){
			$.each(bubbles, function(){
				...
				setTimeout(function(){
					...
					BubbleShoot.Sounds.play("_mp3/pop.mp3",Math.random() * 0.5 + 0.5);
				},delay);
				board.popBubbleAt(bubble.getRow(),bubble.getCol());
				setTimeout(function(){
					bubble.getSprite().remove();					
				},delay + POP_SETUP_DELAY);
				delay += DELAY_PER_BUBBLE_POP;  //orig DELAY_PER_BUBBLE_POP = hard coded 60 
			});
		};
